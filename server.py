...
class RLServerStrategy(FaultTolerantFedAvg):
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        for params in values_aggregated:
            if len(params) != 3:
                raise ValueError("Expected 3 elements (q_tables, states, actions) from each client, got: ", len(params))
            q_tables, states, actions = params
            all_q_tables.append(q_tables)
            all_states.append(states)
            all_actions.append(actions)
...
if __name__ == "__main__":
    ...
    # Guardar historial después del entrenamiento
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
